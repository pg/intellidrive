    ...
    )
{
    ...
    {
        KdPrint(("Intellidrive: Query registry failed, using default values.\n"));
    ...
    )
{
    ...
    case IOCTL_FILE_DISK_OPEN_FILE:
        {
            ...
            if (device_extension->media_in_device)
            {
                KdPrint(("Intellidrive: IOCTL_FILE_DISK_OPEN_FILE: Media already opened\n"));
    ...
    case IOCTL_CDROM_READ_TOC_EX:
        {
            ...
            default:
                KdPrint((
                    "Intellidrive: Unknown format %#x for IOCTL_CDROM_READ_TOC_EX\n",
                    cdrom_toc_ex->Format
                    ));
    ...
    default:
        {
            KdPrint((
                "Intellidrive: Unknown IoControlCode %#x\n",
                io_stack->Parameters.DeviceIoControl.IoControlCode
                ));
    ...
    )
{
	...
	INTELLIDRIVE_CONTEXT intellidriveContext;
    ...
    for (;;)
    {
		...
		while(status == STATUS_TIMEOUT)
		{
			...
			if(IntellidriveExecuteIdleTasks(&intellidriveContext) > 0)
			{
				// More idle tasks remain
            ...
            ))
        {
            ...
            case IRP_MJ_READ:
				...
				*/
				...
				if(IntellidriveReadData(
					&intellidriveContext,
					io_stack->Parameters.Read.ByteOffset.QuadPart,
					io_stack->Parameters.Read.Length,
					buffer) == io_stack->Parameters.Read.Length)
				{
            ...
            case IRP_MJ_DEVICE_CONTROL:
                ...
                case IOCTL_FILE_DISK_OPEN_FILE:
					...
					intellidriveContext = IntellidriveOpenDrive();					
					intellidriveContext.fileHandle = device_extension->file_handle;
                ...
                case IOCTL_FILE_DISK_CLOSE_FILE:
					...
					IntellidriveCloseDrive(&intellidriveContext);
                    break;
    ...
    if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_NO_SUCH_FILE)
    {
        ...
        else
        {
            ...
            if (Irp->IoStatus.Information == FILE_CREATED)
            {
                ...
                if (NT_SUCCESS(status))
                {
                    KdPrint(("Intellidrive: Sparse file created.\n"));
                }
